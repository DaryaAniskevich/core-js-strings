...
function removeFirstOccurrences(str, value) {
  const firstIndex = str.includes(value) ? str.indexOf(value) : str.length;
...
function sumOfCodes(str) {
  ...
  return str
    .split('')
    .reduce((acc, item, index) => acc + str.charCodeAt(index), 0);
}
...
function isPalindrome(str) {
  const newSrt = str.split('').reverse().join('');

  return (
    newSrt.toLowerCase().replaceAll(/[^a-zA-Z0-9]/g, '') ===
    str.toLowerCase().replaceAll(/[^a-zA-Z0-9]/g, '')
  );
}
